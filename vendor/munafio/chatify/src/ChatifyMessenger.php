...
class ChatifyMessenger
{
    ...
    public function parseMessage($prefetchedMessage = null, $id = null)
    {
        ...
        return [
            ...
            'isSender' => ($msg->from_id == Auth::guard('member')->user()->id),
            'seen' => $msg->seen,
        ];
    }
    ...
    {
        return Message::where('from_id', Auth::guard('member')->user()->id)->where('to_id', $user_id)
                    ->orWhere('from_id', $user_id)->where('to_id', Auth::guard('member')->user()->id);
    }
    ...
    public function makeSeen($user_id)
    {
        Message::Where('from_id', $user_id)
                ->where('to_id', Auth::guard('member')->user()->id)
                ->where('seen', 0)
                ->update(['seen' => 1]);
        return 1;
    }
    ...
    {
        return Message::where('from_id', $user_id)->where('to_id', Auth::guard('member')->user()->id)->where('seen', 0)->count();
    }
    ...
    public function getUserWithAvatar($user)
    {
        if ($user->avatar == 'avatar.png' && config('chatify.gravatar.enabled')) {
            ...
            $user->avatar = self::getUserAvatarUrl($user->image);
        }
        return $user;
    }
    ...
    {
        return Favorite::where('user_id', Auth::guard('member')->user()->id)
    ...
    public function makeInFavorite($user_id, $action)
    {
        if ($action > 0) {
            // Star
            $star = new Favorite();
            $star->user_id = Auth::guard('member')->user()->id;
            $star->favorite_id = $user_id;
            $star->save();
            return $star ? true : false;
        } else {
            // UnStar
            $star = Favorite::where('user_id', Auth::guard('member')->user()->id)->where('favorite_id', $user_id)->delete();
            return $star ? true : false;
        }
    }
    ...
    {
        try {
            $msg = Message::where('from_id', auth()->guard('member')->user()->id)->where('id', $id)->firstOrFail();
            if (isset($msg->attachment)) {
                $path = config('chatify.attachments.folder') . '/' . json_decode($msg->attachment)->new_name;
                if (self::storage()->exists($path)) {
                    self::storage()->delete($path);
    ...
    {
        return asset('storage/attachments/'. $attachment_name);
