...
{	
	...
	$.fn.tagsInit=function(option){
		...
		//创建ul
		var oUl=$("<ul id='"+tagper+"_tag_list'></ul>").appendTo('.tag');
		//$('<div style="clear:both;height:0px;"></div>').appendTo(oDiv);//清理浮动
		//标签input
		if($tagdiv.data('input')){
			var note=$tagdiv.data('inputNote');
			//创建input
			var oInput=$("<input type='text' value='"+note+"' title='"+note+"' class='maxwidth100 input-value w-100 ' pla style='padding:15px; max-width: 100%;' id='"+tagper+"_tag_input' />").appendTo(oDiv);
			//给input添加事件
